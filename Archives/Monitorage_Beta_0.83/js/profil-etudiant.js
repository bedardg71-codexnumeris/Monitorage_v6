...
function genererContenuCibleIntervention(da) {
    ...
    if (progression.direction === '→' && indices.R >= seuilRisqueModere) {
        // Stabilité en zone de risque élevé/critique = stagnation problématique
        interpretationProgression = 'Stagnation (progression insuffisante)';
    } else if (progression.direction === '→' && indices.R >= seuilRisqueFaible) {
        // Stabilité en zone de risque modéré = attention
        interpretationProgression = 'Stabilité fragile';
    }
    ...
    // Couleurs badge selon niveau RàI (cohérentes avec Liste et Aperçu)
    let badgeStyle = '';
    let badgeLabel = '';
    if (cibleInfo.niveau === 3) {
        badgeStyle = 'background: #fff3e0; color: #e65100; border: 2px solid #e65100;';
        badgeLabel = 'RàI 3';
    } else if (cibleInfo.niveau === 2) {
        badgeStyle = 'background: #fff9e6; color: #f57c00; border: 2px solid #f57c00;';
        badgeLabel = 'RàI 2';
    } else {
        badgeStyle = 'background: #e8f5e9; color: #2e7d32; border: 2px solid #2e7d32;';
        badgeLabel = 'RàI 1';
    }
    ...
    return `
                ...
                <div style="margin-top: 20px;">
                    <div class="details-calculs-bloc">
                        ...
                        <div class="details-calculs-valeur">
                            ...
                            <strong>⎯</strong> Si la variation est inférieure à 5% → Stagnation du risque
                        </div>
        ...
        <div class="profil-carte">
            ...
            <ul class="info-liste">
                ...
                <li><strong>Pattern :</strong> ${patternTexte}</li>
    ...
    container.innerHTML = `
        <div class="layout-sidebar-2col">
            ...
            <div class="zone-principale" id="profil-contenu-dynamique">
                ...
                <div class="profil-contenu-header">
                    <h2 style="display: flex; justify-content: space-between; align-items: center; margin: 0 0 20px 0;">
                        <span>Suivi de l'apprentissage${genererBadgePratiqueProfil(indices.pratique)}</span>
...
function determinerCibleIntervention(da) {
    ...
    // 5. Stable MAIS risque élevé = stagnation problématique
    const seuilRisqueModere = obtenirSeuil('risque.modere');
    const seuilRisqueFaible = obtenirSeuil('risque.faible');

    if (M === 'Stable' && indices.R >= seuilRisqueModere) {
        return {
            cible: 'Stagnation à risque élevé | Intervention pour sortir de la zone de risque',
            pattern: M,
            niveau: 2,
            couleur: '#ff9800',
            emoji: '🟠'
